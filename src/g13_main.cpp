...
void G13::setStringConfigValue(const std::string& name, const std::string& value) {
    G13_DBG("setStringConfigValue " << name << " = " << Helper::repr(value));
    stringConfigValues[name] = value;
}

std::string G13::MakePipeName(const G13_Device* usb_device, const bool is_input) {
...
LINUX_KEY_VALUE G13::FindG13KeyValue(const std::string& keyname) {
...
G13_State_Key G13::FindInputKeyValue(const std::string& keyname, bool down) {
...
std::string G13::FindInputKeyName(const LINUX_KEY_VALUE v) {
    try {
...
std::string G13::FindG13KeyName(const G13_KEY_INDEX v) {
    try {
